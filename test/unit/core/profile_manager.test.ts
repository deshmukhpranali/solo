...
const cacheDir = getTestCacheDir('ProfileManager')

const argv = ArgvMoc.create()
  .setValue(flags.nodeAliasesUnparsed, 'node1,node2,node4')
  .setValue(flags.cacheDir, cacheDir)
  .setValue(flags.releaseTag, version.HEDERA_PLATFORM_VERSION)
  .setValue(flags.apiPermissionProperties, path.join(cacheDir, 'templates', 'api-permission.properties'))
  .setValue(flags.applicationProperties, path.join(cacheDir, 'templates', 'application.properties'))
  .setValue(flags.bootstrapProperties, path.join(cacheDir, 'templates', 'bootstrap.properties'))
  .setValue(flags.log4j2Xml, path.join(cacheDir, 'templates', 'log4j2.xml'))
  .setValue(flags.settingTxt, path.join(cacheDir, 'templates', 'settings.txt'))
